...
def get_key(api_key_state, key_name):
    ...
    if api_key_state is not None: 
        #first try to get the key from the UI
        api_key = api_key_state.get(key_name, None)               
    if api_key is None: 
        #then try to get it from the OS environment
        api_key = os.environ.get(c.ENV_VAR_NAMES[key_name])
    return api_key

def check_api_key(ss, api_key_state):
    """Set the API key for the selected model"""
    ss.llm.api_key = get_key(api_key_state, ss.llm.provider)
